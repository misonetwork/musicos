...
fun test_new_mono_audio() {
    let audio = audio::new(1, 16, 44100, 1000, walrus_data::new_blob(1), TestIngesterWitness());
    assert_eq!(audio.channels(), 1);
}

#[test]
fun test_new_all_valid_bit_depths() {
    let audio_8 = audio::new(1, 8, 44100, 1000, walrus_data::new_blob(1), TestIngesterWitness());
    assert_eq!(audio_8.bit_depth(), 8);

    let audio_16 = audio::new(1, 16, 44100, 1000, walrus_data::new_blob(1), TestIngesterWitness());
    assert_eq!(audio_16.bit_depth(), 16);

    let audio_24 = audio::new(1, 24, 44100, 1000, walrus_data::new_blob(1), TestIngesterWitness());
    assert_eq!(audio_24.bit_depth(), 24);

    let audio_32 = audio::new(1, 32, 44100, 1000, walrus_data::new_blob(1), TestIngesterWitness());
    assert_eq!(audio_32.bit_depth(), 32);
}

#[test]
fun test_new_samples_at_max() {
    let audio = audio::new(1, 16, 44100, MAX_SAMPLES, walrus_data::new_blob(1), TestIngesterWitness());
...
fun test_duration_ms() {
    ...
    let audio = audio::new(2, 16, 44100, 44100, walrus_data::new_blob(1), TestIngesterWitness());
    assert_eq!(audio.duration_ms(), 1000);

    // 88200 samples at 44100 Hz = exactly 2000 ms
    let audio2 = audio::new(2, 16, 44100, 88200, walrus_data::new_blob(1), TestIngesterWitness());
    assert_eq!(audio2.duration_ms(), 2000);

    // 48000 samples at 48000 Hz = exactly 1000 ms
    let audio3 = audio::new(1, 24, 48000, 48000, walrus_data::new_blob(1), TestIngesterWitness());
